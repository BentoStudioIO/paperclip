...
- **Person.pharmiaUserId**: the Pharmia **better-auth user id** (`ba_user.id`) — the CRM↔app link. Set it whenever the person has a Pharmia account; its presence is how we identify an **Atlas user** in the CRM (combine with `group=PHARMACIST_OWNER` for the owner∩Atlas segment). `group` is single-select so it can't also encode "Atlas user" — that's exactly why the link field exists. Get the id from `pg canary app "SELECT id FROM ba_user WHERE lower(email)='<email>'"`. A helper script `pharmia-twenty-atlas-sync` can fill it by email-match for `app`-tenant signups — but it is **manual / unscheduled** (no cron or timer runs it today) and it fills ONLY `pharmiaUserId`. Don't assume a background job has set this; set it explicitly when you know the account.
- **Person.atlasUsage** (product-usage state): `NEVER` | `ANON_TRIED` | `SIGNED_UP` | `ACTIVE`. Meaning = usage depth (pharmiaUserId presence = *linked*; atlasUsage = *how deep*). **KNOWN GAP: this is NOT auto-derived today** — nothing syncs it from `ba_user` (the atlas-sync script touches only `pharmiaUserId`, never this). Until a real sync exists, it must be set **explicitly** when you have the evidence; otherwise leave it. The "owner ∩ active-Atlas-user" segment therefore relies on whatever was set by hand, not a live feed.
