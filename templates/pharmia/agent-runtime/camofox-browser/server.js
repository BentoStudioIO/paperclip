...
// Export cookies from a user's browser context (Playwright cookies format,
// includes HttpOnly). Optional `?domain=<suffix>` filters to cookies whose
// `domain` field ends with that suffix (e.g. `agendrix.com`).
//
// GET /sessions/:userId/cookies[?domain=example.com]
app.get('/sessions/:userId/cookies', async (req, res) => {
  try {
    ...
    const domainSuffix = typeof req.query.domain === 'string' ? req.query.domain : null;
    if (domainSuffix && domainSuffix.length > 256) {
      return res.status(400).json({ error: 'domain filter too long' });
    }
    ...
    const filtered = domainSuffix
      ? all.filter((c) => typeof c.domain === 'string' && c.domain.endsWith(domainSuffix))
      : all;
    ...
    log('info', 'cookies exported', {
      ...
      domainFilter: domainSuffix ?? null,
    });
    res.json({ ok: true, userId: String(userId), cookies: filtered });
  } catch (err) {
